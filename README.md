# 👨‍💻 Suriyanarayanan G — Full-Stack Developer

**MERN • Flask • Python • React • Next.js • DevOps**

![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=26&duration=3000&color=00C2FF&lines=Full+Stack+Developer;Microservices+Builder;AI+Project+Developer)

[![Email](https://img.shields.io/badge/Email-hsudhan422%40gmail.com-red?style=flat&logo=gmail)](mailto:hsudhan422@gmail.com)
[![Phone](https://img.shields.io/badge/Phone-%2B91%206374057649-blue?style=flat&logo=whatsapp)](tel:+916374057649)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profile-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/suriya31-g)
[![GitHub](https://img.shields.io/badge/GitHub-Profile-black?style=flat&logo=github)](https://github.com/suriya-3108)
[![Portfolio](https://img.shields.io/badge/Portfolio-Website-green?style=flat&logo=vercel)](https://personal-portfolio-gray-five.vercel.app/)

## 🚀 About Me

Passionate **Full-Stack Developer** building scalable web systems using MERN + Flask. Focused on backend performance, automation pipelines, and microservices architecture. Creator of **MicroSculpt AI — Automated Monolith to Microservice Transformer**.

# 🛠️ Tech Stack

![Tech Stack](https://skillicons.dev/icons?i=python,js,cpp,html,css,react,nextjs,redux,tailwind,nodejs,express,flask,mongodb,mysql,git,docker,aws,linux,postman,github,vscode)

# 🧠 Featured Projects

## 🏗️ MicroSculpt AI — Monolith → Microservices Transformation Platform

- AI-driven system to convert monolithic applications into microservices
- AST parsing + dependency graph generation
- ML clustering for service boundary detection
- Auto-generates Flask microservices + Docker templates
- Reduces refactoring effort by 70–80%

**Tech:** Python, Flask, Scikit-learn, NetworkX, Docker  


## 🎬 Hybrid Movie Recommender System

- Hybrid recommendation engine (content + collaborative)
- 85% accuracy
- Flask REST APIs + MongoDB
- Real-time search optimization (2s → 200ms)

**Tech:** HTML, CSS, JavaScript, Flask, MongoDB


## 🤖 AI Personal Chatbot

- NLP chatbot with context memory
- Multi-turn conversation support
- Robust fallback handling

**Tech:** React, Flask, MongoDB, JWT, Tailwind CSS


# 💼 Experience

## Junior Software Engineer Intern — SkillRank

Mar 2025 – Jul 2025 | Remote

- Built MERN + Flask modules
- PDF automation & SIRS extraction
- MongoDB API optimization
- React/Next reusable components
- Agile + CI/CD workflow


# 🎓 Education

B.Tech — Information Technology (2022–2026)  
Perunthalaivar Kamarajar Institute of Engineering and Technology  
CGPA: 8.4 / 10


# 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=suriya-3108&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=suriya-3108&layout=compact&theme=radical)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=suriya-3108&theme=radical)


# 🤝 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/suriya31-g)
[![GitHub](https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github)](https://github.com/suriya-3108)
[![Portfolio](https://img.shields.io/badge/Portfolio-green?style=for-the-badge&logo=vercel)](https://personal-portfolio-gray-five.vercel.app/)

⭐ Explore my repositories and feel free to collaborate!
